...
export default function Inbox() {
  ...
  const handleKeepForLater = (item: InboxItem) => {
    // Remove the item from the inbox
    setItems(items.filter(i => i.id !== item.id))
    
    // Add the item to the content library
    const contentLibraryItems = JSON.parse(localStorage.getItem('contentLibraryItems') || '[]')
    contentLibraryItems.push(item)
    localStorage.setItem('contentLibraryItems', JSON.stringify(contentLibraryItems))
    
    // Optionally, show a notification or feedback to the user
    alert(`Item "${item.header}" has been moved to the Content Library`)
  }
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <header className="bg-white border-b border-gray-200 p-4 sticky top-0 z-10">
            <div className={`mx-auto flex items-center justify-between ${isMobile ? 'max-w-3xl' : 'max-w-5xl'}`}>
              <h1 className="text-xl font-bold text-purple-800">Inbox</h1>
